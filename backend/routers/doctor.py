...
async def doctor_query(request: QueryRequest):
    ...
    if request.mode == "generate":
        ...
        # FIX: Join with medical_records for diagnosis searches
        mapping = {
            ...
            "penicillin": "SELECT p.first_name, p.last_name, a.allergen, a.severity FROM patients p JOIN allergies a ON p.patient_id = a.patient_id WHERE LOWER(a.allergen) LIKE '%penicillin%' AND LOWER(a.severity) = 'severe'",
            "robert smith": "SELECT p.first_name, p.last_name, m.diagnosis, m.record_date FROM patients p JOIN medical_records m ON p.patient_id = m.patient_id WHERE LOWER(p.first_name) LIKE '%robert%' AND LOWER(p.last_name) LIKE '%smith%' LIMIT 50",
            "my patients": f"SELECT DISTINCT p.first_name, p.last_name, p.contact_number FROM patients p JOIN appointments a ON p.patient_id = a.patient_id WHERE a.doctor_id = (SELECT doctor_id FROM doctors LIMIT 1)", # Mock ID for now, ideally get from auth
        }
...
def get_doctor_analytics():
    try:
        # 1. SUMMARY CARDS
        # Assuming doctor_id=1 for demo, or generally all patients if no filter
        ...
        # 3. PATIENT CONDITION (Based on Allergy Severity for now as proxy)
        ...
        # 5. COMPLIANCE (Based on Appointment Status)
